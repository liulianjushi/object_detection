...
def result(value, output_dict):
    detected_list = [0] * len(output_dict["detection_boxes"])
    ious = decide_overlap(output_dict["detection_boxes"], value.values[:,5:9])
    for k,iou in enumerate(ious):
        ...
        if iou[max_iou_id] ==np.max(ious[:,max_iou_id]) and iou[max_iou_id] >0.5 and detected_list[int(max_iou_id)] != 1:
    ...
    for i,det in enumerate(detected_list):
        if det==1:
            continue
        v = value_dict.copy()
        v["图片名称"] = str(value.values[0,0]).split(".")[0]
